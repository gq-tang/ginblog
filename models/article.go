...
// list article
func ListArticle(db sqlx.Queryer, page, offset, status int, title, keywords string) ([]Article, error) {
	var arts []Article
	query := `
	select id,
	title,
	...
	query += " and (title like ? or keywords like ? ) limit ? offset ?"
	start := (page - 1) * offset
	err := sqlx.Select(db, &arts, query, status, title, keywords, offset, start)
	if err != nil {
		return arts, err
	}
	return arts, nil
...
// count article
func CountArticle(db sqlx.Queryer, status int, title, keywords string) (int64, error) {
	var count int64
	query := `select count(id) cnt	from article where status=? `
	if title == "" {
		title = "%"
	} else {
		title = "%" + title + "%"
	...
	err := sqlx.Get(db, &count, query, status, title, keywords)
	if err != nil {
		return 0, err
	}
	return count, nil
